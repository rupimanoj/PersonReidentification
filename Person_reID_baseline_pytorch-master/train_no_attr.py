...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        if(epoch == 0 and opt.freeze_learning):
            freeze_pretrained_model(model)
        if(epoch == 10 and opt.freeze_learning):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val':
                ...
                if (epoch%5 == 4 and epoch < 35) or epoch > 35:
...
exp_lr_scheduler = lr_scheduler.StepLR(optimizer_ft, step_size=30, gamma=0.1) #0.1 gamma earlier
#exp_lr_scheduler = lr_scheduler.MultiStepLR(optimizer_ft, milestones=[35,45,55], gamma=0.33)
######################################################################
# Train and evaluate
# ^^^^^^^^^^^^^^^^^^
#
...
print("checking model folder-------------------", dir_name)
